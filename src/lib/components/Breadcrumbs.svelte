...
<nav class="flex min-w-0 flex-wrap items-center gap-1 text-sm" aria-label="Breadcrumb">
	<a href="/s3" class="rounded px-1.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">S3</a>
	...
	<a href="/s3/{bucket}" class="max-w-[14rem] truncate rounded px-1.5 py-1 text-xs font-medium text-foreground transition-colors hover:bg-muted hover:text-primary">{bucket}</a>
	{#each parts() as part}
		<svg class="size-3 shrink-0 text-border" fill="none" viewBox="0 0 6 10" stroke="currentColor" stroke-width="1.5">
			<path stroke-linecap="round" stroke-linejoin="round" d="M1 1l4 4-4 4"/>
		</svg>

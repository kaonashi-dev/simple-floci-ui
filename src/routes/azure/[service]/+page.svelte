...
<div class="mx-auto w-full max-w-5xl space-y-5 animate-fade-in-up">
	<a href="/azure" class="inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-primary">
	...
	<div class="page-header">
		<div>
			<p class="console-subtle-label">Azure / {data.service.category}</p>
			...
			<div class="flex items-start gap-4">
				...
				<div class="min-w-0">
					<div class="flex flex-wrap items-center gap-2">
						<h2 class="text-lg font-semibold text-foreground">Dedicated Azure view reserved</h2>
					...
					<p class="mt-1 max-w-2xl text-sm text-muted-foreground">
						This route exists so {data.service.name} can grow into its own Floci-AZ explorer instead of being forced into a generic multi-cloud screen.
					</p>
				</div>
			</div>
		</div>
			...
			<div class="space-y-4">
				<div>
					...
					<p class="mt-2 text-sm text-muted-foreground">
						Wire this page to the Floci-AZ {data.service.name} endpoints when the service-specific list, detail, and mutation flows are ready.
					</p>
				</div>

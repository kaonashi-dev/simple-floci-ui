				...
				<tbody>
					{#each filtered as pool}
						<tr class="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors">
							<td class="px-4 py-3">
								<a href="/aws/cognito/{encodeURIComponent(pool.id)}" class="font-medium text-foreground hover:text-primary transition-colors">
									{pool.name}
								</a>
							</td>
							<td class="px-4 py-3">
							...
							<td class="px-4 py-3 text-right">
								<div class="flex items-center justify-end gap-1">
									<Button variant="ghost" size="sm" class="h-7 px-2 text-xs" href="/aws/cognito/{encodeURIComponent(pool.id)}">
										Open
									</Button>
									<Button
										variant="ghost"

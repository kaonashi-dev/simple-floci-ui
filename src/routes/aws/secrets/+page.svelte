						...
						<tr class="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors">
							<td class="px-4 py-3">
								<div class="flex items-center gap-1.5">
									<a href="/aws/secrets/{encodeURIComponent(secret.arn)}" class="font-medium text-foreground hover:text-primary transition-colors">
										{secret.name}
									</a>
							...
							<td class="px-4 py-3 text-right">
								<div class="flex items-center justify-end gap-1">
									<Button variant="ghost" size="sm" class="h-7 px-2 text-xs" href="/aws/secrets/{encodeURIComponent(secret.arn)}">
										Open
									</Button>
									<Button
										variant="ghost"
